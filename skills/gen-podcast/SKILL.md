...
description: >-
  ...
  generate podcast、content to podcast、text to speech dialogue。
---
...
python -m podcast.content_parser_tts merge-audio-files \
    audios/podcast/Large_language_model/0  output.mp3
```
...
| `--category` | `0` | 分类 ID（0=未知 1=科技 2=教育 …） |
| `--is-published` | `False` | 仅 `run` 可用，标记为已发布 |
...
| `python -m podcast.gen_podcast get-source-type <URL>` | 判断来源类型 |
...
├── .env.example                   # 环境变量模板
├── podcast/
│   ├── gen_podcast.py             # 端到端 CLI 入口
│   ├── content_parser_tts.py      # 内容提取 + LLM + TTS
│   ├── insert_podcast.py          # R2 上传 + D1 入库
│   ├── content_parser/
...
│   │   └── table/podcast.py       # LLM 提示词 + 结构化输出
│   ├── aws/upload.py              # R2/S3 上传
│   └── cloudflare/rest_api.py     # D1 REST API
└── audios/                        # 生成的音频输出
```
...
1. **确认环境**：确保 `podcast` 已安装且 `.env` 已配置 `GOOGLE_API_KEY`
2. **确定来源**：从用户请求中提取 URL 或 PDF 路径
3. **选择语言和语音**：根据用户需求选择 `--language` 和 `--role-tts-voices`
4. **执行生成**：
   - 仅需音频 → 使用 `instruct-content-tts`
   - 需要入库发布 → 使用 `gen_podcast run`
5. **返回结果**：告知用户生成的音频文件路径，或在线播客链接（若已发布）
